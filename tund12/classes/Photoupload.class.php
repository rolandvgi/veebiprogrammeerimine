<?php
  class Photoupload {
	...
	public function readExif(){
		//vältikamsk probleeme(warning), kasutan @märki
	...
	public function createThumbnail($directory, $size){
		...
		imagecopyresampled($myThumbnail, $this->myTempImage, 0, 0, $cutX, $cutY, $size, $size, $cutSize, $cutSize);
			...
			$notice = $this->savePhoto($myThumbnail, $targetFile);
		//thumbnail kirjutatakse pildifailiks
		return $notice;
	}
	
	public function saveNormalPhoto($targetFile) {
		return $this->savePhoto($this->myImage, $targetFile);
	}

	public function savePhoto($image, $targetFile){
	  $notice = "";
