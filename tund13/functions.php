<?php
  ...
  $database = "if18_Roland_va_1";
	...
	function listalluudised(){
	$msgHTML = "";
    $mysqli = new mysqli($GLOBALS["serverHost"],$GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
    $stmt = $mysqli->prepare("SELECT userid, title, content, expire FROM vpnews");
	echo $mysqli->error;
	$stmt->bind_result($titleFromDb, $contentFromDb, $expiredateFromDb);
	$stmt->execute();
	while($stmt->fetch()){
		$msgHTML .= "<p>" .$titleFromDb . " " .$expiredateFromDb. "</p> \n";
		$msgHTML .= "<p>" .$contentFromDb . "</p> \n";
	}
	$stmt->close();
	$mysqli->close();
	return $msgHTML;
	...
	function uudistelisamine($title, $content, $expiredate){
		$notice = "";
		$mysqli = new mysqli($GLOBALS["serverHost"],$GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		$stmt = $mysqli->prepare("INSERT INTO vpnews (userid, title, content, expire) VALUES(?, ?, ?, ?)");
		echo $mysqli->error;
		$stmt->bind_param("issi", $_SESSION["userId"], $title, $content, $expiredate);
		if ($stmt->execute()){
		  $notice = 'Uudis on lisatud';
		} else {
		  $notice = "Uudise lisamisel tekkis tõrge: " .$stmt->error;
		}
		$stmt->close();
		$mysqli->close();
	return $notice;
  ...
  // function listprivatephotos($privacy){
	// $html = "";
	// $mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
  //   $stmt = $mysqli->prepare("SELECT filename, alttext FROM vpphotos WHERE privacy=? AND userid=? AND deleted is NULL");
	// echo $mysqli->error;
  ...
  function listmyprivatephotos($page, $limit){
	$html = "";
		...
		$skip = ($page - 1) * $limit;
	$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
    $stmt = $mysqli->prepare("SELECT filename, alttext FROM vpphotos WHERE privacy=?  AND userid=? AND deleted is NULL LIMIT ?,?");// 3,6
	echo $mysqli->error;
  ...
  function listpublicphotospage($page, $limit){ //vaadadadassadsad
	$html = "";
	$privacy = 2;
	$skip = ($page - 1) * $limit;
	$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	...
	$stmt->bind_result($idFromDb, $firstnameFromDb, $lastnameFromDb, $filenameFromDb, $alttextFromDb, $avgphotoratingFromDb);
	$stmt->execute();
	while($stmt->fetch()){
		...
		if ($avgphotoratingFromDb == null){
			$average = "Hinnang puudub";
		} else {
			$average = 'Hinne: ' . $avgphotoratingFromDb;
			
		}
		$html .= '<p id="score' .$idFromDb. '">'.$average.'</p> ';
		$html .= '</div>';
	}
  ...
  function storeuserprofile($desc, $bgcol, $txtcol, $picId){
	...
	if($stmt->fetch()){
		...
		//kui on pilt lisatud
		if(!empty($picId)){
		  $stmt = $mysqli->prepare("UPDATE vpuserprofiles SET description=?, bgcolor=?, txtcolor=?, photoid=? WHERE userid=?");
		  echo $mysqli->error;
		...
		if(!empty($picId)){
		  $stmt = $mysqli->prepare("INSERT INTO vpuserprofiles (userid, description, bgcolor, txtcolor, photoid) VALUES(?,?,?,?,?)");
		  echo $mysqli->error;
  ...
  function showmyprofile(){
	$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
    $stmt = $mysqli->prepare("SELECT description, bgcolor, txtcolor, photoid FROM vpuserprofiles WHERE userid=?");
	echo $mysqli->error;
	$stmt->bind_param("i", $_SESSION["userId"]);
