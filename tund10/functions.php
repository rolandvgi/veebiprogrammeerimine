...
function insertprofilepic($profilepic){
	$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	$stmt = $mysqli->prepare("INSERT INTO vpuserprofiles (photoid) VALUES (?)");
	echo $mysqli->error;
	$stmt->bind_param("ii", $_SESSION["userId"], $profilepic);
	if($stmt->execute()){
...
function readavatar(){
	$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
  $stmt = $mysqli->prepare("SELECT photoid FROM vpuserprofiles WHERE photoid=?");
	echo $mysqli->error;
	...
	if($stmt->fetch()){
		...
		$_SESSION["profilepic"] = "../vp_picfiles/vp_user_gengeric.png";
	}
	$stmt->close();
	$mysqli->close();
  }
  ...
  $stmt = $mysqli->prepare("SELECT description, bgcolor, txtcolor, photoid FROM vpuserprofiles WHERE userid=?");
	...
	if($stmt->fetch()){
		...
		$stmt->bind_param("sssii", $desc, $bgcol, $txtcol, $picId, $_SESSION["userId"]);
			...
			$stmt->$mysqli->prepare("UPDATE vpuserpofiles SET description=?, bgcolor=?, txtcolor=? WHERE userid=?");
			echo $mysqli->error;
		}
		if($stmt->execute()){
		...
		$stmt->close();
		if(!empty($picId)){
		  $stmt = $mysqli->prepare("INSERT INTO vpuserprofiles (userid, description, bgcolor, txtcolor, pphotoid) VALUES(?,?,?,?,?)");
		  echo $mysqli->error;
		  $stmt->bind_param("isssi", $_SESSION["userId"], $desc, $bgcol, $txtcol, $picId);
		} else {
		  //INSERT INTO vpusers3 (firstname, lastname, birthdate, gender, email, password) VALUES(?,?,?,?,?,?)"		  echo $mysqli->error;
		}
