<?php
  ...
  function listprivatephotospage($privacy){
	$html = "";
	$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
    $stmt = $mysqli->prepare("SELECT filename, alttext FROM vpphotos WHERE privacy=? AND userid=? AND deleted is NULL LIMIT 6");// 3,6
	echo $mysqli->error;
	$stmt->bind_param("ii", $privacy, $_SESSION["userId"]);
	$stmt->bind_result($filenameFromDb, $alttextFromDb);
	$stmt->execute();
  ...
  function listpublicphotospage($privacy){
	$html = "";
	$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
    $stmt = $mysqli->prepare("SELECT filename, alttext FROM vpphotos WHERE privacy<=? AND deleted is NULL LIMIT 6");// 3,6
	echo $mysqli->error;
